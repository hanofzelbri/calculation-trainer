...
const Calculator: React.FC = () => {
    ...
    const handleCheck = () => {
        const maxResultLength = getMaxResultLength();
        const userAnswer = Array.from({ length: maxResultLength }, (_, i) => answerRefs.current[i]?.value || '').join('');
        const result = checkAnswer(parseInt(userAnswer, 10));
        showFeedbackAnimation(result);

        if (result) {
            // Clear both the input refs and state values
            answerRefs.current.forEach((input) => {
                if (input) input.value = '';
            });
            setAnswerValues(Array(maxResultLength).fill(''));
            setCarryValues(Array(maxResultLength).fill(''));
        } else {
